...
export default class Home extends React.Component
{
    ...
    componentDidMount()
    {
      let token="Token "+window.sessionStorage.getItem("token")
        setInterval(()=>
        {
          axios.get('http://192.168.43.158/watermodule/TanksReading/Sump/', { headers: { Authorization:token} })
 .then(response => {
     // If request is good...
     
    chart.dials.dial[0].value=parseInt((response.data[0].Tank_Reading_Recieved/200)*100)
    this.setState(
      {}
    )
  })
